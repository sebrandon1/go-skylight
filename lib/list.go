...
func (c *Client) GetList(ctx context.Context, frameID, listID string) (*List, error) {
	req, err := newRequest(ctx, "GET", fmt.Sprintf("%s/frames/%s/lists/%s", c.effectiveURL(), pathSeg(frameID), pathSeg(listID)))
	if err != nil {
...
func (c *Client) CreateList(ctx context.Context, frameID string, list ListData) (*List, error) {
	if list.Kind == "" {
		list.Kind = "to_do"
	}
	if list.Color == "" {
	...
	req, err := newRequestWithBody(ctx, "POST", fmt.Sprintf("%s/frames/%s/lists", c.effectiveURL(), pathSeg(frameID)), list)
	if err != nil {
...
func (c *Client) UpdateList(ctx context.Context, frameID, listID string, list ListData) (*List, error) {
	req, err := newRequestWithBody(ctx, "PUT", fmt.Sprintf("%s/frames/%s/lists/%s", c.effectiveURL(), pathSeg(frameID), pathSeg(listID)), list)
	if err != nil {
...
func (c *Client) DeleteList(ctx context.Context, frameID, listID string) error {
	req, err := newRequest(ctx, "DELETE", fmt.Sprintf("%s/frames/%s/lists/%s", c.effectiveURL(), pathSeg(frameID), pathSeg(listID)))
	if err != nil {
...
func (c *Client) AddListItem(ctx context.Context, frameID, listID string, item ListItemData) (*ListItem, error) {
	...
	req, err := newRequestWithBody(ctx, "POST", fmt.Sprintf("%s/frames/%s/lists/%s/list_items", c.effectiveURL(), pathSeg(frameID), pathSeg(listID)), send)
	if err != nil {
...
func (c *Client) UpdateListItem(ctx context.Context, frameID, listID, itemID string, item ListItemData) (*ListItem, error) {
	send := listItemSendData{
		Label:    item.Title,
		Position: item.Position,
	}
	...
	req, err := newRequestWithBody(ctx, "PUT", fmt.Sprintf("%s/frames/%s/lists/%s/list_items/%s", c.effectiveURL(), pathSeg(frameID), pathSeg(listID), pathSeg(itemID)), send)
	if err != nil {
...
func (c *Client) DeleteListItem(ctx context.Context, frameID, listID, itemID string) error {
	req, err := newRequest(ctx, "DELETE", fmt.Sprintf("%s/frames/%s/lists/%s/list_items/%s", c.effectiveURL(), pathSeg(frameID), pathSeg(listID), pathSeg(itemID)))
	if err != nil {
...
func (c *Client) ClearCompletedListItems(ctx context.Context, frameID, listID string) (int, error) {
	...
	for _, item := range list.Items {
		if item.Completed {
			if err := c.DeleteListItem(ctx, frameID, listID, item.ID); err != nil {
...
func (c *Client) OrganizeGroceryList(ctx context.Context, frameID, listID string) error {
	req, err := newRequest(ctx, "POST", fmt.Sprintf("%s/frames/%s/lists/%s/organize", c.effectiveURL(), pathSeg(frameID), pathSeg(listID)))
	if err != nil {
...
func (c *Client) OrderGroceryList(ctx context.Context, frameID, listID, retailer string) (string, error) {
	body := struct {
		Retailer string `json:"retailer,omitempty"`
	}{Retailer: retailer}

	req, err := newRequestWithBody(ctx, "POST", fmt.Sprintf("%s/frames/%s/lists/%s/order", c.effectiveURL(), pathSeg(frameID), pathSeg(listID)), body)
	if err != nil {
...
func (c *Client) CreateTaskBoxItem(ctx context.Context, frameID string, item TaskBoxItemData) (*TaskBoxItem, error) {
	reqBody := TaskBoxItemRequest{TaskBoxItem: item}

	req, err := newRequestWithBody(ctx, "POST", fmt.Sprintf("%s/frames/%s/task_box_items", c.effectiveURL(), pathSeg(frameID)), reqBody)
	if err != nil {
